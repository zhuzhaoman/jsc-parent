 ...
 **/
@Service
public class TcpFlowModelSystemManagerSendingSystemPolicyServiceImpl implements SystemManagerSendingSystemPolicyService {
    ...
    public Object configDataEncapsulation(SystemBO systemBO){
        ...
        sendSystemManagerDTO.setMessageCode(MessageCodeEnum.SYSTEM_CONFIG_TCP_FLOW_MODEL.getReqCode());
    ...
    public void recordUserLog(SystemBO systemBO) {
        StringBuilder content = new StringBuilder(MessageCodeEnum.SYSTEM_CONFIG_TCP_FLOW_MODEL.getMsg());
        try {
            JSONObject params = JSONObject.parseObject(JSONObject.toJSONString(systemBO.getParam()));
            content.append("【")
                    .append("模式:").append(params.getInteger("m_u32CreateFlag") == 0 ? "syn包" : "首包")
                    .append("】");
        } catch (Exception e) {
            e.printStackTrace();
        }
