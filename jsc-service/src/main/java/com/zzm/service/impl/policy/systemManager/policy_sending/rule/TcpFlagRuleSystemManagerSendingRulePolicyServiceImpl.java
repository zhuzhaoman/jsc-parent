 ...
 **/
@Service
public class TcpFlagRuleSystemManagerSendingRulePolicyServiceImpl extends BaseSystemManagerSendingPolicyServiceImpl implements SystemManagerSendingRulePolicyService {
    ...
    public Object addDataEncapsulation(RuleBO ruleBO) {
        TcpFlagRuleBO tcpFlagRuleBO = JSON.parseObject(JSON.toJSONString(ruleBO.getParam()), TcpFlagRuleBO.class);

        System.out.println("进入tcp-flag");
        ...
        SendSystemManagerDTO sendSystemManagerDTO = new SendSystemManagerDTO(
                ...
                MessageCodeEnum.TCPFLAG_RULE_ADD.getReqCode(),
                ruleBO.getUsername(),
                ruleBO.getDomainId(),
    ...
    public Object delDataEncapsulation(RuleBO ruleBO) {
        ...
        int ruleType = ruleNumber > 1 ?
                MessageCodeEnum.TCPFLAG_RULE_MORE_DEL.getReqCode() :
                MessageCodeEnum.TCPFLAG_RULE_SOLO_DEL.getReqCode();
    ...
    public void recordUserLog(RuleBO ruleBO) {
        ...
        if ("add".equals(ruleBO.getRuleAction())) {
            content.append(MessageCodeEnum.TCPFLAG_RULE_ADD.getMsg());
        } else if ("del".equals(ruleBO.getRuleAction())) {
            RuleDelBO ruleDelBO = JSON.parseObject(JSON.toJSONString(ruleBO.getParam()), RuleDelBO.class);
            String ruleMsg = ruleDelBO.getM_u32RuleNum() > 1 ?
                    MessageCodeEnum.TCPFLAG_RULE_MORE_DEL.getMsg() :
                    MessageCodeEnum.TCPFLAG_RULE_SOLO_DEL.getMsg();
            content.append(ruleMsg);
        } else if ("find".equals(ruleBO.getRuleAction())) {
            content.append(MessageCodeEnum.TCP_FLAG_GET_ONE.getMsg());
        }
