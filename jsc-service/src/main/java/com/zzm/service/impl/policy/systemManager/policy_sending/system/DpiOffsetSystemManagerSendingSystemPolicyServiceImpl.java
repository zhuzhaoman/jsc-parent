 ...
 **/
@Service
public class DpiOffsetSystemManagerSendingSystemPolicyServiceImpl implements SystemManagerSendingSystemPolicyService {
    ...
    public Object configDataEncapsulation(SystemBO systemBO){
        ...
        sendSystemManagerDTO.setMessageCode(MessageCodeEnum.SYSTEM_CONFIG_DPI_OFFSET.getReqCode());
    ...
    public void recordUserLog(SystemBO systemBO) {
        StringBuilder content = new StringBuilder(MessageCodeEnum.SYSTEM_CONFIG_DPI_OFFSET.getMsg());
        try {
            JSONObject params = JSONObject.parseObject(JSONObject.toJSONString(systemBO.getParam()));
            content.append("【")
                    .append("偏移量:").append(params.getInteger("m_u32DeviationValue"))
                    .append("】");
        } catch (Exception e) {
            e.printStackTrace();
        }
