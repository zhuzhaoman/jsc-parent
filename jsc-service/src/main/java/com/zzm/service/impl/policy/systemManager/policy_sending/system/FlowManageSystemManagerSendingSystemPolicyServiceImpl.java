 ...
 **/
@Service
public class FlowManageSystemManagerSendingSystemPolicyServiceImpl implements SystemManagerSendingSystemPolicyService {
    ...
    public Object configDataEncapsulation(SystemBO systemBO){
        ...
        sendSystemManagerDTO.setMessageCode(MessageCodeEnum.SYSTEM_CONFIG_FLOW_MANAGE.getReqCode());
    ...
    public void recordUserLog(SystemBO systemBO) {
        StringBuilder content = new StringBuilder(MessageCodeEnum.SYSTEM_CONFIG_FLOW_MANAGE.getMsg());
        try {
            JSONObject params = JSONObject.parseObject(JSONObject.toJSONString(systemBO.getParam()));
            content.append("【")
                    .append("建流标志:").append(params.getInteger("m_u32Type") == 1 ? "TCP" : "UDP")
                    .append("】");
        } catch (Exception e) {
            e.printStackTrace();
        }
