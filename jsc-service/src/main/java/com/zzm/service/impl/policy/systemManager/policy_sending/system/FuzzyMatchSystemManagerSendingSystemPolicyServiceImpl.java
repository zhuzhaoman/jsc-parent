 ...
 **/
@Service
public class FuzzyMatchSystemManagerSendingSystemPolicyServiceImpl implements SystemManagerSendingSystemPolicyService {
    ...
    public Object configDataEncapsulation(SystemBO systemBO){
        ...
        sendSystemManagerDTO.setMessageCode(MessageCodeEnum.SYSTEM_CONFIG_FUZZY_MATCH.getReqCode());
    ...
    public void recordUserLog(SystemBO systemBO) {
        StringBuilder content = new StringBuilder(MessageCodeEnum.SYSTEM_CONFIG_FUZZY_MATCH.getMsg());
        try {
            JSONObject params = JSONObject.parseObject(JSONObject.toJSONString(systemBO.getParam()));
            content.append("【")
                    .append("开关:").append(params.getInteger("m_u32FuzzyEnable") == 0 ? "关" : "开")
                    .append("】");
        } catch (Exception e) {
            e.printStackTrace();
        }
