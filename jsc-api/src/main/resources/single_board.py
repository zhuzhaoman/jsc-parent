...
access_controller = paramList[6]

command_list = ['cd /u01/install', 'sh InstallationAtOnce.sh', str(install_or_update), str(device_id),
                str(user_num), str(unit_type), str(box_type), str(access_controller)]
...
class SshClient():
    ...
    def send(self):
        ...
        # list = ["cd /u01/install", "sh InstallationAtOnce.sh", "1", "1", "2", "1", "0", "0"]
        # list = ["cd /u01/install", "sh InstallationAtOnce.sh", "", "", ""]
...
if __name__ == "__main__":
    ssh = SshClient()
    if ssh.ssh_login(host_ip="192.168.17.135", username="root", password="JSC@3passok") == 1000:
        ssh.send()
        time.sleep(300)
        ssh.ssh_logout()
